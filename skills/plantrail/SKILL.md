...
5. `plantrail next` (`-n 5` for more) to choose what comes next.
...
- Repo moved or cloned elsewhere and the thread no longer resumes? `plantrail link tN` links this location (add `--prune` to drop paths that no longer exist). Moves are healed automatically when the origin URL still matches.
